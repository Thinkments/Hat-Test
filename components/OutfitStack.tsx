...
const OutfitStack: React.FC<OutfitStackProps> = ({ outfitHistory, onRemoveLastGarment }) => {
  return (
    <div className="flex flex-col">
      <h2 className="text-xl font-serif tracking-wider text-gray-800 border-b border-gray-400/50 pb-2 mb-3">Current Look</h2>
      <div className="space-y-2">
        {outfitHistory.map((layer, index) => {
          const is3DModel = index === 0 && Object.keys(layer.poseImages).length > 1;
          return (
            <div
            ...
            >
              <div className="flex items-center overflow-hidden">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {is3DModel && (
                      <span className="text-xs font-bold text-indigo-600 bg-indigo-100 px-2 py-0.5 rounded-full">3D</span>
                    )}
                  </div>
              </div>
        ...
        {outfitHistory.length === 1 && (
            <p className="text-center text-sm text-gray-500 pt-4">Your current look will appear here. Select a hat from the gallery below.</p>
        )}
      </div>
    </div>
  );
